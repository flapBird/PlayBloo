          ...
          <TableBody>
            {categories.map(cat => (
              <TableRow key={cat.id}>
                ...
                <TableCell>
                  {editing === cat.id ? (
                    <div className="flex gap-1"><Button variant="ghost" size="icon" onClick={() => update(cat.id)}><Check className="h-4 w-4" /></Button><Button variant="ghost" size="icon" onClick={() => setEditing(null)}><X className="h-4 w-4" /></Button></div>
                  ) : (
                    <div className="flex gap-1"><Button variant="ghost" size="icon" onClick={() => { setEditing(cat.id); setEditName(cat.name); setEditSlug(cat.slug); setEditDesc(cat.description || ""); setEditSort(String(cat.sort_order)); }}><Edit className="h-4 w-4" /></Button><Button variant="ghost" size="icon" onClick={() => remove(cat.id)}><Trash2 className="h-4 w-4" /></Button></div>
                  )}
                </TableCell>
              </TableRow>
            ))}
